...
5. Работа декораторов в модуле `decorators`:
   ...
   который определяет, куда будут записываться логи (в файл или в консоль):
        ...
        ```
6. Работа функций в модуле `transactions` (не забудьте установить библиотеки `pandas` и `openpyxl`):
    - Функция `read_transactions_csv` принимает на вход путь до файла `.csv` и возвращает список словарей
   с данными по транзакциям. В случае отсутствия файла или пустого содержимого, выдает соответствующие сообщения.
    - Функция `read_transactions_xlsx` принимает на вход путь до файла `.xlsx` и возвращает список словарей
   с данными по транзакциям. В случае отсутствия файла или пустого содержимого, выдает соответствующие сообщения.
